...
export default function ProductForm() {
    ...
    const handleSubmit = (e) => {
        ...
        // Panggil fetchData untuk memperbarui productList
        fetchData();

        // Kirim data produk ke REST API
        postData(newDataWithUUID);
    ...
    const updateProduct = async (productData) => {
        try {
          const response = await axios.put(`${apiUrl}/${productData.id}`, productData);
        ...
        if (confirm) {
            axios.delete(`${apiUrl}/${id}`)
                .then((response) => {
                    ...
                    // menghapus data dari local state
    ...
    const handleDeleteProduct = async (id) => {
        try {
          // Make a DELETE request to your API endpoint with the product ID
          const response = await axios.delete(`https://your-api-url/products/${id}`);
          if (response.status === 200) {
            // Product successfully deleted, update the product list
            const updatedProductList = productList.filter((product) => product.id !== id);
            dispatch(setProductList(updatedProductList));
            dispatch(setDeleteProductId(null)); // Clear the deleteProductId
    ...
    // MockAPI
    const apiUrl = 'https://652be38fd0d1df5273eedbbd.mockapi.io/products';
        ...
        <form className="product-form col-8" onSubmit={handleSubmit} name="form-product">
            ...
            {editingItem && (
                ...
                <div className="edit-button">
                    <button
                        onClick={() => updateProduct(editingItem)}
                        type="button"
